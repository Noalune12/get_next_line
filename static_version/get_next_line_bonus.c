...
/*   Updated: 2024/11/27 12:54:50 by lbuisson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	content[OPEN_MAX][BUFFER_SIZE + 1];
	char		*line;
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 1024)
		return (NULL);
	if (content[fd][0])
		line = join_line_content(line, content[fd]);
	line = read_file_and_extract_line(fd, content[fd], line);
	if (!line)
	{
		ft_memset(content[fd], '\0', BUFFER_SIZE + 1);
		return (NULL);
	}
	update_buffer(content[fd]);
	return (line);
}
